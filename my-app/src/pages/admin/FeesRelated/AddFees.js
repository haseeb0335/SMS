...
import { 
    ...
    Grid, Card, CardContent, InputAdornment, FormHelperText, useTheme, useMediaQuery
} from "@mui/material";
...
const AddFees = () => {
    ...
    const downloadMonthlyPDF = (className, monthName, fees) => {
        const doc = new jsPDF();
        doc.setFontSize(18);
        doc.text(`Fee Report: ${className}`, 14, 15);
        doc.setFontSize(12);
    ...
    const groupedFees = feesList.reduce((acc, fee) => {
    ...
    return (
        <Box sx={{ p: { xs: 2, md: 4 }, bgcolor: '#f8fafc', minHeight: '100vh' }}>
            ...
            <Paper elevation={0} sx={{ 
                ...
                <Grid container spacing={3}>
                    <Grid item xs={12} sm={6}>
                        <TextField 
                            select label="Class" value={selectedClass} fullWidth 
                    ...
                    <Grid item xs={12} sm={6}>
                        <TextField 
                            select label="Student" value={selectedStudent} fullWidth 
                            disabled={!selectedClass} onChange={(e) => setSelectedStudent(e.target.value)}
                            InputProps={{
                                startAdornment: (
                                    <InputAdornment position="start">
                    ...
                    <Grid item xs={12} sm={6}>
                        <TextField 
                            ...
                            InputProps={{
                                startAdornment: (
                                    <InputAdornment position="start">
                                        <Typography sx={{ fontWeight: 'bold', color: '#10b981', fontSize: '0.9rem' }}>PKR</Typography>
                                    </InputAdornment>
                                ),
                            }}
                        />
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 4 }}>
                    <Button 
                        variant="contained" fullWidth={isMobile} onClick={handleAddFee} 
                        sx={{ 
                            px: 8, py: 1.8, borderRadius: '12px', textTransform: 'none', fontWeight: 700,
            ...
            {Object.entries(groupedFees).map(([className, months]) => (
                        ...
                        <Box key={month} sx={{ mb: 4, ml: { md: 2 } }}>
                            <Paper sx={{ borderRadius: 3, overflow: 'hidden', border: '1px solid #e2e8f0' }} elevation={0}>
                                <Stack 
                                    ...
                                    sx={{ p: 2, bgcolor: '#f8fafc', borderBottom: '1px solid #e2e8f0', gap: 2 }}
                                >
                                    ...
                                    <Button 
                                        ...
                                        onClick={() => downloadMonthlyPDF(className, month, fees)}
                                        sx={{ bgcolor: 'white', textTransform: 'none', fontWeight: 600 }}
                                    >
                                        PDF Report
                                    </Button>
                                </Stack>
                                ...
                                {isMobile ? (
                                    <Box sx={{ p: 2 }}>
                                        {fees.map((f, i) => (
                                            <Card key={f._id} variant="outlined" sx={{ mb: 2, borderRadius: 2 }}>
                                                <CardContent sx={{ pb: '12px !important' }}>
                                                    <Stack direction="row" justifyContent="space-between" sx={{ mb: 1 }}>
                                                        <Typography variant="caption" sx={{ fontWeight: 700, color: '#94a3b8' }}>#{i + 1}</Typography>
                                                        <Box>
                                                            <IconButton size="small" onClick={() => handleEditFee(f)}><EditIcon color="primary" fontSize="small" /></IconButton>
                                                            <IconButton size="small" onClick={() => handleDeleteFee(f._id)}><DeleteIcon color="error" fontSize="small" /></IconButton>
                                                        </Box>
                                                    </Stack>
                                                    <Typography variant="body1" sx={{ fontWeight: 700, color: '#1e293b' }}>{f.studentName}</Typography>
                                                    <Stack direction="row" justifyContent="space-between" mt={1}>
                                                        <Typography variant="body2" sx={{ fontWeight: 800, color: '#10b981' }}>Rs. {f.amount}</Typography>
                                                        <Typography variant="body2" color="textSecondary">{new Date(f.date).toLocaleDateString()}</Typography>
                                                    </Stack>
                                                </CardContent>
                                            </Card>
                                        ))}
                                    ...
                                    <TableContainer>
                                        <Table size="medium">
                                            <TableHead sx={{ bgcolor: '#ffffff' }}>
                                                <TableRow>
                                                    <TableCell sx={{ fontWeight: 'bold', color: '#64748b' }}>#</TableCell>
                                                    <TableCell sx={{ fontWeight: 'bold', color: '#64748b' }}>Student Name</TableCell>
                                                    <TableCell sx={{ fontWeight: 'bold', color: '#64748b' }}>Amount (PKR)</TableCell>
                                                    ...
                                                    <TableRow key={f._id} hover>
                                                        ...
                                                        <TableCell align="right">
                                                            <Stack direction="row" spacing={1} justifyContent="flex-end">
                                                                <IconButton size="small" sx={{ bgcolor: '#eff6ff' }} onClick={() => handleEditFee(f)}><EditIcon color="primary" fontSize="small" /></IconButton>
                                                                <IconButton size="small" sx={{ bgcolor: '#fef2f2' }} onClick={() => handleDeleteFee(f._id)}><DeleteIcon color="error" fontSize="small" /></IconButton>
                                                            </Stack>
                                                        </TableCell>
                                                    </TableRow>
                                                ))}
